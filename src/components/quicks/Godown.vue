<template>
    ...
    >
        ...
        >
            <down-two theme="outline" size="16" />
        </button>
    </div>
</template>
<script>
import { defineComponent, nextTick, ref } from '@vue/runtime-core'
import { DownTwo } from '@icon-park/vue-next'
...
export default defineComponent({
    ...
    setup() {
        ...
        const handleBttonClick = event => {
            Selector('.' + blockAttr.value.className).down()
            setQuick({
                display: false
            })
            nextTick(() => {
                addRecord()
                // 更新按钮状态
                if (blockAttr.value.className && Selector('.' + blockAttr.value.className).hasNextElement()) {
                    hsactive.value = true
                } else {
